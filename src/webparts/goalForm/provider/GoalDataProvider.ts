...
export default class GoalDataProvider implements IGoalDataProvider {
    ...
    public getGoalById(id: number): Promise<Goal> {
        return this.Web.lists.getByTitle(this.TitleList).items.getById(id).select(
            GoalPropertiesConst.ID,GoalPropertiesConst.Points,GoalPropertiesConst.Title,GoalPropertiesConst.Details, GoalPropertiesConst.SkillId,GoalPropertiesConst.SkillTitle
        ).expand(GoalPropertiesConst.Skill).get().then((item:Item)=>{
            let goal : Goal = {
                ...
                Skill : item[UEPropertiesConst.Skill], 
              };
